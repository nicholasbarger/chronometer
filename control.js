...
chronometer.selectedDate = null;

chronometer.generateCalendar = function (date) {
    var output = '';
    var m = moment(date);
    var datesToDisplay = [];
    
    // get start of active month
    var startOfMonth = m.clone().startOf('month');
    ...
    for (var i = weekDayOfStartOfMonth; i > 0; i--) {
        datesToDisplay.push(m.clone().subtract(i, 'days'));
    }

    // add days from this month as long as it doesn't exceed max number (42)
    var totalDaysInMonth = m.daysInMonth();
    for (var i = 0; i < (42 - weekDayOfStartOfMonth); i++) {
        datesToDisplay.push(startOfMonth.clone().add(i, 'days'));
    }

    // generate top of table
    output += '<table><thead><tr><td><a href="#">&lt;</a></td><th colspan="5">January</th><td><a href="#">&gt;</a></td></tr><tr><th>Su</th><th>M</th><th>Tu</th><th>W</th><th>Th</th><th>F</th><th>Sa</th></tr></thead><tbody>';
    ...
    for (var i = 0; i <= (datesToDisplay.length - 1) / 7; i++) {
        ...
        for (var j = 0; j < 7; j++) {
            var cm = datesToDisplay[(i * 7) + j];

            output += "<td onclick=\"chronometer.selectDay(this, '" + cm.toISOString() + "')\"";
            
            // check if the date is the selected date
            if (chronometer.selectedDate != null && cm.date() == chronometer.selectedDate.date()) {
            ...
            // check if next month
            else if (cm.month() > m.month()) {
                output += 'class="next-month">';
            }
            // check if today
            else if (cm.date() == m.date()) {
                output += 'class="today">';
            }
            // set date display
            else {
    ...
    // generate bottom of table
    output += '</tbody><tfoot><tr><td colspan="7"><a href="javascript:void(0);" onclick="chronometer.selectToday()">Today</a></td></tr></tfoot></table>';

    // check options to see if we should include time selector
    if(chronometer.options.includeTime == true) {
        output += generateTimeSelector();
    }

    // assign formatted html to calendar
    $('.chronometer').after(output);
};

chronometer.refreshOutput = function () {
    // use moment to get date
    var output = chronometer.selectedDate.format(chronometer.options.outputFormat);

    // todo - change this selector
    $('.chronometer').val(output);
};


chronometer.selectDay = function (e, date) {
    var output = null;

    // set selected date to be used when updating timezone
    chronometer.selectedDate = moment(date);

    // clear css from past selected element
    $('.selectedDate').removeClass('selectedDate');
    
    // decorate the element with selected class
    $(e).addClass('selectedDate');

    // assign formatted output to text field
    chronometer.refreshOutput();
};

chronometer.selectTime = function (e) {
    var timeValues = $(e).val().split(':');
    var hours = timeValues[0];
    var minutes = timeValues[1];

    chronometer.selectedDate.hours(hours).minutes(minutes);
    chronometer.refreshOutput();
};

chronometer.selectTimezone = function (tz) {
    chronometer.selectedDate.tz(tz);
    chronometer.refreshOutput();
}

chronometer.selectToday = function () {
    chronometer.selectDay(new Date());
};

function generateTimeSelector() {
    return '<table class="chronometer-time">' +
            '<thead>' +
                '<tr>' +
                    '<th>Time</th>' +
                '</tr>' +
            ...
            '<tbody>' +
                '<tr>' +
                    '<td><input type="time" onblur="chronometer.selectTime(this)" /></td>' +
                '</tr>' +
                '<tr>' +
                    '<td>' +
                        '<select onchange="chronometer.selectTimezone(this.value)">' +
                            ...
                            '<option value="Antarctica/DumontDUrville">(GMT+10:00) Antarctica/DumontDUrville (Dumont-dUrville Time)</option>' +
