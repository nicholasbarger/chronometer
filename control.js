(function ($) {
    $.fn.chronometer = function (date, options) {
        ...
        return this.each(function () {
            ...
            // select time and refresh output
            this.selectTime = function (elem) {
                var timeValues = $(elem).val().split(':');
                var hours = timeValues[0];
                var minutes = timeValues[1];

                _selectedDate.hours(hours).minutes(minutes);
                self.refreshOutput();
            };

            // select time zone and refresh output
            this.selectTimezone = function (tz) {
                _selectedDate.tz(tz);
                self.refreshOutput();
            };
            ...
            function generateTimeSelector() {
                return '<table class="chronometer-time">' +
                        ...
                        '<tbody>' +
                            '<tr>' +
                                '<td>' +
                                    '<select onchange="' + id + '.selectTimezone(this.value)">' +
                            ...
                            '</tr>' +
                            '<tr>' +
                                '<td><input type="time" onblur="' + id + '.selectTime(this)" /></td>' +
                            '</tr>' +
                        '</tbody>' +
                    '</table>';
            }
            ...
            this.generateCalendar = function() {
                ...
                output += '</tbody><tfoot><tr><td colspan="7"><a href="javascript:void(0);" onclick="' + id + '.selectToday()">Today</a></td></tr></tfoot></table>';

                // check options to see if we should include time selector
                if (_settings.includeTime == true) {
